#!/usr/bin/env bash
# Generates platforms/kimi-code/adapters/ with flat-namespace adapter skills.
# Each adapter inlines (a) the using-gpowers preamble, (b) the source skill body.
set -euo pipefail

: "${GPOWERS_HOME:?GPOWERS_HOME required}"
ADAPTERS_DIR="$GPOWERS_HOME/platforms/kimi-code/adapters"
...
---
name: $adapter_name
description: "$orig_desc (gpowers adapter for Kimi Code)"
...
# Flat manifest for Kimi Code discovery
find "$ADAPTERS_DIR" -mindepth 1 -maxdepth 1 -type d | sort | awk -F/ '{print $NF}' \
  | jq -R . | jq -s '{adapters: .}' \
  > "$GPOWERS_HOME/platforms/kimi-code/kimi-code-skills.json"

echo "Kimi Code adapters generated under $ADAPTERS_DIR"
