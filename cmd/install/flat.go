...
func generateFlatAdapters(gpowersHome, platform string) error {
	adaptersDir := filepath.Join(gpowersHome, "platforms", platform, "adapters")
...
name: %s
description: "%s (gpowers adapter for %s)"
...
%s
`, adapterName, desc, platformTitle, mod, orig, mod, preamble, mod, orig, body)
	...
	manifestFile := fmt.Sprintf("%s-skills.json", platform)
	out, err := json.MarshalIndent(map[string]interface{}{"adapters": adapterNames}, "", "  ")
	if err != nil {
		return fmt.Errorf("marshal adapters: %w", err)
	}
	return os.WriteFile(filepath.Join(gpowersHome, "platforms", platform, manifestFile), append(out, '\n'), 0644)
}

func registerFlatSkills(gpowersHome, platform string) error {
	...
	adaptersDir := filepath.Join(gpowersHome, "platforms", platform, "adapters")
	skillsDir := expandPath(shape.InstallLinkTarget)

	if err := os.MkdirAll(skillsDir, 0755); err != nil {
		return fmt.Errorf("create %s skills dir: %w", platform, err)
	}
	...
	fmt.Printf("[install] %s skills registered in: %s\n", platform, skillsDir)
	return nil
}
