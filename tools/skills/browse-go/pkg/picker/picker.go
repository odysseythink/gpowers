// Package picker provides auth and HTML for the cookie-picker UI.
// It is intentionally separate from pkg/server to avoid import cycles.
package picker
...
const (
	codeTTLMs     = 30_000    // 30 seconds
	sessionTTLMs  = 3_600_000 // 1 hour
	cookieName    = "browse_picker"
)

// Auth state
var (
	codeMu      sync.Mutex
	pendingCodes map[string]int64 // code -> expiry timestamp (ms)

	sessionMu     sync.Mutex
	validSessions map[string]int64 // session -> expiry timestamp (ms)
)

func init() {
	pendingCodes = make(map[string]int64)
	validSessions = make(map[string]int64)
}

// GenerateCode creates a new one-time code for opening the cookie picker.
func GenerateCode() string {
	code := randomHex(32)
	codeMu.Lock()
	pendingCodes[code] = time.Now().UnixMilli() + codeTTLMs
	codeMu.Unlock()
	return code
}

// ConsumeCode validates a one-time code and consumes it (single use).
// On success it returns a new session ID.
func ConsumeCode(code string) (string, bool) {
	codeMu.Lock()
	expiry, ok := pendingCodes[code]
	if !ok || time.Now().UnixMilli() > expiry {
		delete(pendingCodes, code)
		codeMu.Unlock()
		return "", false
	}
	delete(pendingCodes, code) // one-time use
	codeMu.Unlock()

	session := randomHex(32)
	sessionMu.Lock()
	validSessions[session] = time.Now().UnixMilli() + sessionTTLMs
	sessionMu.Unlock()
	return session, true
}

// ValidateSession checks if a session cookie value is valid and not expired.
func ValidateSession(session string) bool {
	sessionMu.Lock()
	defer sessionMu.Unlock()
	expiry, ok := validSessions[session]
	if !ok {
		return false
	}
	if time.Now().UnixMilli() > expiry {
		delete(validSessions, session)
		return false
	}
	return true
}

// HasActive returns true while the picker still has a live code or session.
func HasActive() bool {
	now := time.Now().UnixMilli()

	codeMu.Lock()
	for code, expiry := range pendingCodes {
		if expiry <= now {
			delete(pendingCodes, code)
		}
	}
	...
	sessionMu.Lock()
	for session, expiry := range validSessions {
		if expiry <= now {
			delete(validSessions, session)
		}
	}
	activeSessions := len(validSessions)
	sessionMu.Unlock()

	codeMu.Lock()
	activeCodes := len(pendingCodes)
	codeMu.Unlock()

	return activeCodes > 0 || activeSessions > 0
}

// SessionCookieName returns the name of the picker session cookie.
func SessionCookieName() string { return cookieName }

// ExtractSession reads the session cookie from a request.
...
// GetHTML returns the self-contained cookie picker HTML page.
func GetHTML(port int) string {
	baseURL := fmt.Sprintf("http://127.0.0.1:%d", port)
	html := strings.ReplaceAll(pickerHTMLTemplate, "{{BASE_URL}}", baseURL)
	html = strings.ReplaceAll(html, "{{PORT}}", fmt.Sprintf("%d", port))
	return html
}

// pickerHTMLTemplate is the self-contained cookie picker UI.
// It is a port of the TS cookie-picker-ui.ts, using vanilla JS and inline CSS.
const pickerHTMLTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
<p class="subtitle">Select the domains of cookies you want to import to Browse. You'll be able to browse those sites with the same login as your other browser.</p>
...
(function() {
  ...
  function showBanner(msg, type, retryFn) {
    ...
    html += '<button class="banner-close" id="banner-close">&times;</button>';
  ...
  function renderSourceDomains() {
    ...
    const filtered = query ? allDomains.filter(d => d.domain.toLowerCase().includes(query)) : allDomains;
    if (filtered.length === 0) {
      $sourceDomains.innerHTML = '<div class="imported-empty">' + (query ? 'No matching domains' : 'No cookie domains found') + '</div>';
      $sourceFooter.textContent = '';
      return;
    }
    let html = '';
