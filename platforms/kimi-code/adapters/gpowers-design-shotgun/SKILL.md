---
name: gpowers-design-shotgun
description: "Design shotgun: generate multiple AI design variants, open a comparison board, (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/design-shotgun/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-shotgun","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"design-shotgun","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"design-shotgun","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
variant, call `$(gpowers-path home)/bin/gpowers-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `$(gpowers-path home)/bin/gpowers-taste-update rejected <variant-path>`.
...
**Why /tmp/ then cp?** In observed sessions, `$D generate --output $(gpowers-path home)/...`
