---
name: gpowers-retro
description: "Weekly engineering retrospective. Analyzes commit history, work patterns, (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/retro/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $(gpowers-path home)/bin/gpowers-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $(gpowers-path home)/bin/gpowers-learnings-search --limit 10 2>/dev/null || true
fi
```
...
```bash
[ -f $(gpowers-path home)/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `$(gpowers-path home)/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat $(gpowers-path home)/greptile-history.md 2>/dev/null || true
...
cat $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
...
**Greptile signal (if history exists):** Read `$(gpowers-path home)/greptile-history.md` (fetched in Step 1, command 8). Filter entries within the retro time window by date. Count entries by type: `fix`, `fp`, `already-fixed`. Compute signal ratio: `(fix + already-fixed) / (fix + already-fixed + fp)`. If no entries exist in the window or the file doesn't exist, skip the Greptile metric row. Skip unparseable lines silently.
...
**Skill Usage (if analytics exist):** Read `$(gpowers-path analytics)/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:
...
**Eureka Moments (if logged):** Read `$(gpowers-path analytics)/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Note:** Only include the `greptile` field if `$(gpowers-path home)/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
```bash
DISCOVER_BIN=""
[ -x $(gpowers-path home)/bin/gpowers-global-discover ] && DISCOVER_BIN=$(gpowers-path home)/bin/gpowers-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x $(gpowers-path home)/bin/gpowers-global-discover ] && DISCOVER_BIN=$(gpowers-path home)/bin/gpowers-global-discover
[ -z "$DISCOVER_BIN" ] && which gpowers-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which gpowers-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/gpowers-global-discover.ts ] && DISCOVER_BIN="bun run bin/gpowers-global-discover.ts"
...
Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/gpowers-discover-stderr
```

Read the stderr output from `/tmp/gpowers-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: gpowers-global-discover" + "feat: /retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t $(gpowers-path home)/retros/global-*.json 2>/dev/null | head -5
```
...
```bash
mkdir -p $(gpowers-path home)/retros
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls $(gpowers-path home)/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `$(gpowers-path home)/retros/global-${today}-${next}.json`:
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `$(gpowers-path home)/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
