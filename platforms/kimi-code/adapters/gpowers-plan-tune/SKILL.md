---
name: gpowers-plan-tune
description: "Self-tuning question sensitivity + developer psychographic for gstack (v1: observational). (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/plan-tune/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
7. **"Turn it off" / "disable"** → `$(gpowers-path home)/bin/gpowers-config set question_tuning false`
8. **"Turn it on" / "enable"** → `$(gpowers-path home)/bin/gpowers-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$($(gpowers-path home)/bin/gpowers-config get question_tuning 2>/dev/null || echo "false")
...
3. If A or B: enable:
   ```bash
   $(gpowers-path home)/bin/gpowers-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `$(gpowers-path home)/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   $(gpowers-path home)/bin/gpowers-developer-profile --read >/dev/null
...
```bash
$(gpowers-path home)/bin/gpowers-developer-profile --profile
```
...
- Show the vibe (archetype) from `gpowers-developer-profile --vibe` — the
...
4. Write:
   ```bash
   $(gpowers-path home)/bin/gpowers-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
$(gpowers-path home)/bin/gpowers-developer-profile --gap
```
...
```bash
$(gpowers-path home)/bin/gpowers-question-preference --stats
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
eval "$($(gpowers-path home)/bin/gpowers-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
$(gpowers-path home)/bin/gpowers-developer-profile --profile | bun -e "
