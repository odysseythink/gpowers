---
name: gpowers-learn
description: "Manage project learnings. Review, search, prune, and export what gstack (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/learn/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"learn","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
$(gpowers-path home)/bin/gpowers-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
$(gpowers-path home)/bin/gpowers-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
$(gpowers-path home)/bin/gpowers-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
$(gpowers-path home)/bin/gpowers-learnings-search --limit 50 2>/dev/null
```
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
