---
name: gpowers-sync-gbrain
description: "Keep gbrain current with this repo's code and refresh agent search (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/sync-gbrain/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `$(gpowers-path home)/` indexing (the existing `gpowers-gbrain-source-wireup`
...
`$(gpowers-path home)/bin/gpowers-gbrain-sync.ts`.
...
```bash
$(gpowers-path home)/bin/gpowers-gbrain-detect 2>/dev/null
```
...
also runs locally** in local-stdio MCP mode — `gpowers-memory-ingest` shells
...
`$(gpowers-path home)/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `gpowers-brain-sync`
...
Also check the per-repo trust policy. If `gpowers-gbrain-repo-policy get` for
...
```bash
bun run $(gpowers-path home)/bin/gpowers-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `$(gpowers-path home)/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`$(gpowers-path home)/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' $(gpowers-path home)/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- gpowers-gbrain-search-guidance:start -->` through
`<!-- gpowers-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <gpowers-code-{repo_slug}> (page_count=<N>)
  $(gpowers-path home) source. OK   <gpowers-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `$(gpowers-path home)/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `$(gpowers-path home)/.brain-allowlist`
(which is for `$(gpowers-path home)/` brain-sync only). On a different Mac with a synced
