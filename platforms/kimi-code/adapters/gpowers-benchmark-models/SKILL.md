---
name: gpowers-benchmark-models
description: "Cross-model benchmark for gstack skills. Runs the same prompt through Claude, (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/benchmark-models/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark-models","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"benchmark-models","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
You are running the `/benchmark-models` workflow. Wraps the `gpowers-model-benchmark` binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.
...
```bash
BIN="$HOME/$(gpowers-path home)/bin/gpowers-model-benchmark"
[ -x "$BIN" ] || BIN="$(gpowers-path home)/bin/gpowers-model-benchmark"
[ -x "$BIN" ] || { echo "ERROR: gpowers-model-benchmark not found. Run ./setup in the gstack install dir." >&2; exit 1; }
echo "BIN: $BIN"
```
...
- **Options:**
  - A) Save to `$(gpowers-path home)/benchmarks/<date>-<skill-or-prompt-slug>.json`. Completeness: 10/10.
