---
name: gpowers-open-gstack-browser
description: "Launch GStack Browser — AI-controlled Chromium with the sidebar extension baked in. (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/open-gstack-browser/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"open-gpowers-browser","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"open-gpowers-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"open-gpowers-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /open-gpowers-browser — Launch GStack Browser
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/$(gpowers-path home)/extension/manifest.json" ] && _EXT_PATH="$_ROOT/$(gpowers-path home)/extension"
[ -z "$_EXT_PATH" ] && [ -f "$HOME/$(gpowers-path home)/extension/manifest.json" ] && _EXT_PATH="$HOME/$(gpowers-path home)/extension"
