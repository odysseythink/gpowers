---
name: gpowers-plan-ceo-review
description: "CEO/founder-mode plan review. Rethink the problem, find the 10-star product, (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/plan-ceo-review/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"plan-ceo-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t $(gpowers-path project)/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)" && mkdir -p $(gpowers-path project)/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p $(gpowers-path project)/ceo-plans/archive
# For each stale plan: mv $(gpowers-path project)/ceo-plans/{old-plan}.md $(gpowers-path project)/ceo-plans/archive/
```

Write to `$(gpowers-path project)/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```bash
mkdir -p $(gpowers-path analytics)
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> $(gpowers-path analytics)/spec-review.jsonl 2>/dev/null || true
```
...
**Persist the result:**
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
rm -f $(gpowers-path project)/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
`$(gpowers-path home)/` (user config directory, not project files). The skill preamble
already writes to `$(gpowers-path home)/sessions/` and `$(gpowers-path analytics)/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard in /ship.

```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `$(gpowers-path home)/projects/` only (local, personal reference)
- **C)** Skip
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
