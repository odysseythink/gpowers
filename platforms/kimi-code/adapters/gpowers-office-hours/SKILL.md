---
name: gpowers-office-hours
description: "YC Office Hours — two modes. Startup mode: six forcing questions that expose (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/office-hours/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/$(gpowers-path home)/browse/dist/browse" ] && B="$_ROOT/$(gpowers-path home)/browse/dist/browse"
[ -z "$B" ] && B="$HOME/$(gpowers-path home)/browse/dist/browse"
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
4. **List existing design docs for this project:**
   ...
   ls -t $(gpowers-path project)/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" $(gpowers-path project)/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `$(gpowers-path home)/projects/`.
...
```bash
CODEX_PROMPT_FILE=$(mktemp /tmp/gpowers-codex-oh-XXXXXXXX.txt)
```
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/$(gpowers-path home)/design/dist/design" ] && D="$_ROOT/$(gpowers-path home)/design/dist/design"
[ -z "$D" ] && D="$HOME/$(gpowers-path home)/design/dist/design"
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
_DESIGN_DIR="$(gpowers-path project)/designs/mockup-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
SKETCH_FILE="/tmp/gpowers-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/gpowers-sketch.png
```
...
The screenshot file at `/tmp/gpowers-sketch.png` can be referenced by downstream skills
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t $(gpowers-path project)/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `$(gpowers-path home)/projects/{slug}/{user}-{branch}-design-{datetime}.md`.
...
```bash
mkdir -p $(gpowers-path analytics)
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> $(gpowers-path analytics)/spec-review.jsonl 2>/dev/null || true
```
...
```bash
PROFILE=$($(gpowers-path home)/bin/gpowers-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
**Builder Journey Summary** (session 5+): Auto-generate `$(gpowers-path home)/builder-journey.md`
...
Auto-generate updated `$(gpowers-path home)/builder-journey.md` with narrative arc. Open it.
...
```bash
mkdir -p $(gpowers-path analytics)
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
```
...
The design doc at `$(gpowers-path home)/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
