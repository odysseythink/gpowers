---
name: gpowers-plan-design-review
description: "Designer's eye plan review — interactive, like CEO and Eng review. (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/plan-design-review/SKILL.md
...
<!-- SOURCE: $GPOWERS_HOME/roles/skills/plan-design-review/SKILL.md -->
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
`$(gpowers-path project)/designs/` (user config directory, not project files).
...
- `mkdir -p $(gpowers-path project)/designs/...`
...
**Log the result:**
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/gpowers-ideal-<dimension>.png
```
...
|----------------|-------------|-----------|-------|
| [screen name]  | $(gpowers-path project)/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
