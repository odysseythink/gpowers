---
name: gpowers-design-html
description: "Design finalization: generates production-quality Pretext-native HTML/CSS. (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/design-html/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-html","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"design-html","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"design-html","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_CEO=$(ls -t $(gpowers-path project)/ceo-plans/*.md 2>/dev/null | head -1)
[ -n "$_CEO" ] && echo "CEO_PLAN: $_CEO" || echo "NO_CEO_PLAN"
```

```bash
setopt +o nomatch 2>/dev/null || true
_APPROVED=$(ls -t $(gpowers-path project)/designs/*/approved.json 2>/dev/null | head -1)
[ -n "$_APPROVED" ] && echo "APPROVED: $_APPROVED" || echo "NO_APPROVED"
```

```bash
setopt +o nomatch 2>/dev/null || true
_VARIANTS=$(ls -t $(gpowers-path project)/designs/*/variant-*.png 2>/dev/null | head -1)
[ -n "$_VARIANTS" ] && echo "VARIANTS: $_VARIANTS" || echo "NO_VARIANTS"
```

```bash
setopt +o nomatch 2>/dev/null || true
_FINALIZED=$(ls -t $(gpowers-path project)/designs/*/finalized.html 2>/dev/null | head -1)
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/$(gpowers-path home)/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/$(gpowers-path home)/design-html/vendor/pretext.js"
[ -z "$_PRETEXT_VENDOR" ] && [ -f $(gpowers-path home)/design-html/vendor/pretext.js ] && _PRETEXT_VENDOR=$(gpowers-path home)/design-html/vendor/pretext.js
...
`$(gpowers-path project)/designs/<screen-name>-YYYYMMDD/finalized.html`

For framework output, save to:
`$(gpowers-path project)/designs/<screen-name>-YYYYMMDD/finalized.[tsx|svelte|vue]`
...
$B screenshot /tmp/gpowers-verify-mobile.png --width 375
$B screenshot /tmp/gpowers-verify-tablet.png --width 768
$B screenshot /tmp/gpowers-verify-desktop.png --width 1440
```
