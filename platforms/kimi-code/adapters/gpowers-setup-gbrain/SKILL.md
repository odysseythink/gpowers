---
name: gpowers-setup-gbrain
description: "Set up gbrain for this coding agent: install the CLI, initialize a (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/setup-gbrain/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$(gpowers-path home)/bin/gpowers-gbrain-detect
```
...
>   ❌ Destructive — existing config moved to ~/.gbrain/config.json.gpowers-bak-{ts}
...
**If A (Retry)**: re-run `$(gpowers-path home)/bin/gpowers-gbrain-detect`
...
```bash
BACKUP="$HOME/.gbrain/config.json.gpowers-bak-$(date +%s)"
...
```bash
$(gpowers-path home)/bin/gpowers-gbrain-install
```
...
```bash
. $(gpowers-path home)/bin/gpowers-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | $(gpowers-path home)/bin/gpowers-gbrain-supabase-verify -
```
...
```bash
orgs=$($(gpowers-path home)/bin/gpowers-gbrain-supabase-provision list-orgs --json)
```
...
```bash
trap 'echo ""; echo "gpowers-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
...
```bash
result=$($(gpowers-path home)/bin/gpowers-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
$(gpowers-path home)/bin/gpowers-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$($(gpowers-path home)/bin/gpowers-gbrain-supabase-provision \
...
**4c. Verify via gpowers-gbrain-mcp-verify.** Run the helper; capture the
classified JSON output:

```bash
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  $(gpowers-path home)/bin/gpowers-gbrain-mcp-verify "$MCP_URL")
...
- `URL_FORM_SUPPORTED` (`true|false`) — passed to `gpowers-artifacts-init` in
...
```bash
$(gpowers-path home)/bin/gpowers-gbrain-install || exit $?
...
When A was picked, Step 3 already ran (via gpowers-gbrain-install) and Step 4
...
```bash
current_tier=$($(gpowers-path home)/bin/gpowers-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  $(gpowers-path home)/bin/gpowers-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
`gpowers-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.gpowers-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
$(gpowers-path home)/bin/gpowers-artifacts-init --url-form-supported "$URL_FORM"
$(gpowers-path home)/bin/gpowers-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`gpowers-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `gpowers-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `$(gpowers-path home)/`,
...
try:
    ...
    pass
")
$(gpowers-path home)/bin/gpowers-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `$(gpowers-path home)/.git` yet) so the user sees the failure rather than silently
...
this machine's transcripts indexed, they pull from your `gpowers-artifacts-$USER`
repo (set up in Step 7) on whatever schedule they prefer. Set
`gpowers-config set transcript_ingest_mode off` and continue to Step 8.
...
> it in prior commits. Use `gpowers-transcript-prune` to delete in bulk;
...
After answer:
```bash
$(gpowers-path home)/bin/gpowers-config set transcript_ingest_mode <choice>
$(gpowers-path home)/bin/gpowers-gbrain-sync --full --no-brain-sync
```
...
hook runs `gpowers-gbrain-sync --incremental --quiet` on every skill
...
<!-- gpowers-gbrain-search-guidance:start -->

GBrain is set up and synced on this machine. The agent should prefer gbrain
over Grep when the question is semantic or when you don't know the exact
identifier yet. Two indexed corpora available via the `gbrain` CLI:
- This repo's code (registered as `gpowers-code-<repo>` source).
- `$(gpowers-path home)/` curated memory (registered as `gpowers-brain-<user>` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source gpowers-brain-<user>`

Grep is still right for known exact strings, regex, multiline patterns, and
file globs. The brain auto-syncs incrementally on every gstack skill start.
Run `/sync-gbrain` to force-refresh, `/sync-gbrain --full` for full reindex.

<!-- gpowers-gbrain-search-guidance:end -->
```
...
```bash
$(gpowers-path home)/bin/gpowers-gbrain-detect 2>/dev/null || true
$(gpowers-path home)/bin/gpowers-config get transcript_ingest_mode 2>/dev/null || echo "off"
$(gpowers-path home)/bin/gpowers-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f $(gpowers-path home)/.gbrain-sync-state.json ] && cat $(gpowers-path home)/.gbrain-sync-state.json || echo "{}"
```
...
- If user picked B (No): `N/A declined at Step 4d` — `gpowers-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
gpowers-memory-ingest now persists staged transcripts to
`$(gpowers-path home)/transcripts/run-<pid>-<ts>/` and gpowers-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir $(gpowers-path home)/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf $(gpowers-path home)/.setup-gbrain.lock.d` if
