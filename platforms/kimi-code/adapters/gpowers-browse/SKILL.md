---
name: gpowers-browse
description: "Fast headless browser for QA testing and site dogfooding. Navigate any URL, interact with (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/browse/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"browse","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"browse","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
`$B` is the browse binary (resolved from `$_ROOT/$(gpowers-path home)/browse/dist/browse` or `$(gpowers-path home)/browse/dist/browse`).
