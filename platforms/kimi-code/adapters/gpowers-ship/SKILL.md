---
name: gpowers-ship
description: "Ship workflow: detect + merge base branch, run tests, review diff, bump VERSION, (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/ship/SKILL.md
...
```bash
_UPD=$($(gpowers-path home)/bin/gpowers-update-check 2>/dev/null || $(gpowers-path home)/bin/gpowers-update-check 2>/dev/null || true)
[ -n "$_UPD" ] && echo "$_UPD" || true
mkdir -p $(gpowers-path home)/sessions
touch $(gpowers-path home)/sessions/"$PPID"
_SESSIONS=$(find $(gpowers-path home)/sessions -mmin -120 -type f 2>/dev/null | wc -l | tr -d ' ')
find $(gpowers-path home)/sessions -mmin +120 -type f -exec rm {} + 2>/dev/null || true
_PROACTIVE=$($(gpowers-path home)/bin/gpowers-config get proactive 2>/dev/null || echo "true")
_PROACTIVE_PROMPTED=$([ -f $(gpowers-path home)/.proactive-prompted ] && echo "yes" || echo "no")
_BRANCH=$(git branch --show-current 2>/dev/null || echo "unknown")
echo "BRANCH: $_BRANCH"
_SKILL_PREFIX=$($(gpowers-path home)/bin/gpowers-config get skill_prefix 2>/dev/null || echo "false")
echo "PROACTIVE: $_PROACTIVE"
echo "PROACTIVE_PROMPTED: $_PROACTIVE_PROMPTED"
echo "SKILL_PREFIX: $_SKILL_PREFIX"
source <($(gpowers-path home)/bin/gpowers-repo-mode 2>/dev/null) || true
REPO_MODE=${REPO_MODE:-unknown}
echo "REPO_MODE: $REPO_MODE"
_LAKE_SEEN=$([ -f $(gpowers-path home)/.completeness-intro-seen ] && echo "yes" || echo "no")
echo "LAKE_INTRO: $_LAKE_SEEN"
_TEL=$($(gpowers-path home)/bin/gpowers-config get telemetry 2>/dev/null || true)
_TEL_PROMPTED=$([ -f $(gpowers-path home)/.telemetry-prompted ] && echo "yes" || echo "no")
_TEL_START=$(date +%s)
_SESSION_ID="$$-$(date +%s)"
echo "TELEMETRY: ${_TEL:-off}"
echo "TEL_PROMPTED: $_TEL_PROMPTED"
_EXPLAIN_LEVEL=$($(gpowers-path home)/bin/gpowers-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$($(gpowers-path home)/bin/gpowers-config get question_tuning 2>/dev/null || echo "false")
echo "QUESTION_TUNING: $_QUESTION_TUNING"
mkdir -p $(gpowers-path analytics)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
for _PF in $(find $(gpowers-path analytics) -maxdepth 1 -name '.pending-*' 2>/dev/null); do
  if [ -f "$_PF" ]; then
    if [ "$_TEL" != "off" ] && [ -x "$(gpowers-path home)/bin/gpowers-telemetry-log" ]; then
      $(gpowers-path home)/bin/gpowers-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true
    fi
...
if [ -f "$_LEARN_FILE" ]; then
  ...
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    $(gpowers-path home)/bin/gpowers-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
if [ -f CLAUDE.md ] && grep -q "## Skill routing" CLAUDE.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
_ROUTING_DECLINED=$($(gpowers-path home)/bin/gpowers-config get routing_declined 2>/dev/null || echo "false")
echo "HAS_ROUTING: $_HAS_ROUTING"
echo "ROUTING_DECLINED: $_ROUTING_DECLINED"
_VENDORED="no"
if [ -d ".claude/skills/gstack" ] && [ ! -L ".claude/skills/gstack" ]; then
  if [ -f "$(gpowers-path home)/VERSION" ] || [ -d "$(gpowers-path home)/.git" ]; then
    _VENDORED="yes"
  fi
fi
echo "VENDORED_GSTACK: $_VENDORED"
echo "MODEL_OVERLAY: claude"
_CHECKPOINT_MODE=$($(gpowers-path home)/bin/gpowers-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$($(gpowers-path home)/bin/gpowers-config get checkpoint_push 2>/dev/null || echo "false")
...
In plan mode, allowed because they inform the plan: `$B`, `$D`, `codex exec`/`codex review`, writes to `$(gpowers-path home)/`, writes to the plan file, and `open` for generated artifacts.
...
If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gpowers-*` names. Disk paths stay `$(gpowers-path home)/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `$(gpowers-path home)/gpowers-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `$(gpowers-path home)/.feature-prompted-continuous-checkpoint`: AskUserQuestion for Continuous checkpoint auto-commits. If accepted, run `$(gpowers-path home)/bin/gpowers-config set checkpoint_mode continuous`. Always touch marker.
- Missing `$(gpowers-path home)/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `$(gpowers-path home)/bin/gpowers-config set explain_level terse`.

Always run (regardless of choice):
```bash
rm -f $(gpowers-path home)/.writing-style-prompt-pending
touch $(gpowers-path home)/.writing-style-prompted
```
...
touch $(gpowers-path home)/.completeness-intro-seen
```
...
If A: run `$(gpowers-path home)/bin/gpowers-config set telemetry community`
...
If B→A: run `$(gpowers-path home)/bin/gpowers-config set telemetry anonymous`
If B→B: run `$(gpowers-path home)/bin/gpowers-config set telemetry off`

Always run:
```bash
touch $(gpowers-path home)/.telemetry-prompted
```
...
If A: run `$(gpowers-path home)/bin/gpowers-config set proactive true`
If B: run `$(gpowers-path home)/bin/gpowers-config set proactive false`

Always run:
```bash
touch $(gpowers-path home)/.proactive-prompted
```
...
If B: run `$(gpowers-path home)/bin/gpowers-config set routing_declined true` and say they can re-enable with `gpowers-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via AskUserQuestion unless `$(gpowers-path home)/.vendoring-warned-$SLUG` exists:

> This project has gstack vendored in `$(gpowers-path home)/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r $(gpowers-path home)/`
2. Run `echo '$(gpowers-path home)/' >> .gitignore`
3. Run `$(gpowers-path home)/bin/gpowers-team-init required` (or `optional`)
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)" 2>/dev/null || true
touch $(gpowers-path home)/.vendoring-warned-${SLUG:-unknown}
```
...
if [ -f "$HOME/.gpowers-artifacts-remote.txt" ]; then
  _BRAIN_REMOTE_FILE="$HOME/.gpowers-artifacts-remote.txt"
else
  _BRAIN_REMOTE_FILE="$HOME/.gpowers-brain-remote.txt"
fi
_BRAIN_SYNC_BIN="$(gpowers-path home)/bin/gpowers-brain-sync"
_BRAIN_CONFIG_BIN="$(gpowers-path home)/bin/gpowers-config"
...
if [ -f "$_BRAIN_REMOTE_FILE" ] && [ ! -d "$_GPOWERS_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" = "off" ]; then
  ...
  if [ -n "$_BRAIN_NEW_URL" ]; then
    ...
    echo "ARTIFACTS_SYNC: run 'gpowers-brain-restore' to pull your cross-machine artifacts (or 'gpowers-config set artifacts_sync_mode off' to dismiss forever)"
  fi
fi
...
If A/B and `$(gpowers-path home)/.git` is missing, ask whether to run `gpowers-artifacts-init`. Do not block the skill.

At skill END before telemetry:

```bash
"$(gpowers-path home)/bin/gpowers-brain-sync" --discover-new 2>/dev/null || true
"$(gpowers-path home)/bin/gpowers-brain-sync" --once 2>/dev/null || true
```
...
`/context-restore` reads `[gpowers-context]`; `/ship` squashes WIP commits into clean commits.
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$(gpowers-path home)/bin/gpowers-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$(gpowers-path home)/bin/gpowers-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
Before building anything unfamiliar, **search first.** See `$(gpowers-path home)/ETHOS.md`.
- **Layer 1** (tried and true) — don't reinvent. **Layer 2** (new and popular) — scrutinize. **Layer 3** (first principles) — prize above all.

**Eureka:** When first-principles reasoning contradicts conventional wisdom, name it and log:
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> $(gpowers-path analytics)/eureka.jsonl 2>/dev/null || true
```
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
`$(gpowers-path analytics)/`, matching preamble analytics writes.
...
rm -f $(gpowers-path analytics)/.pending-"$_SESSION_ID" 2>/dev/null || true
# Session timeline: record skill completion (local-only, never sent anywhere)
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
# Local analytics (gated on telemetry setting)
if [ "$_TEL" != "off" ]; then
echo '{"skill":"SKILL_NAME","duration_s":"'"$_TEL_DUR"'","outcome":"OUTCOME","browse":"USED_BROWSE","session":"'"$_SESSION_ID"'","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
# Remote telemetry (opt-in, requires binary)
if [ "$_TEL" != "off" ] && [ -x $(gpowers-path home)/bin/gpowers-telemetry-log ]; then
  $(gpowers-path home)/bin/gpowers-telemetry-log \
...
In plan mode before ExitPlanMode: if the plan file lacks `## GSTACK REVIEW REPORT`, run `$(gpowers-path home)/bin/gpowers-review-read` and append the standard runs/status/findings table. With `NO_REVIEWS` or empty, append a 5-row placeholder with verdict "NO REVIEWS YET — run `/autoplan`". If a richer report exists, skip.
...
```bash
$(gpowers-path home)/bin/gpowers-review-read
```
...
**Review tiers:**
- **Eng Review (required by default):** The only review that gates shipping. Covers architecture, code quality, tests, performance. Can be disabled globally with \`gpowers-config set skip_eng_review true\` (the "don't bother me" setting).
...
For Design Review: run `source <($(gpowers-path home)/bin/gpowers-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
[ -f $(gpowers-path project)/no-test-bootstrap ] && echo "BOOTSTRAP_DECLINED"
```
...
If user picks H → write `$(gpowers-path project)/no-test-bootstrap` and continue without tests.
...
If user picks C → write `$(gpowers-path project)/no-test-bootstrap`. Tell user: "If you change your mind later, delete `$(gpowers-path project)/no-test-bootstrap` and re-run." Continue without tests.
...
Write to `$(gpowers-path home)/projects/{slug}/{user}-{branch}-ship-test-plan-{datetime}.md`:
...
# Compute project slug for $(gpowers-path home)/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$(gpowers-path home)/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" "$(gpowers-path project)/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
```bash
_CROSS_PROJ=$($(gpowers-path home)/bin/gpowers-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $(gpowers-path home)/bin/gpowers-learnings-search --limit 10 --query "release ship version changelog merge pr" --cross-project 2>/dev/null || true
else
  $(gpowers-path home)/bin/gpowers-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
fi
```
...
If A: run `$(gpowers-path home)/bin/gpowers-config set cross_project_learnings true`
If B: run `$(gpowers-path home)/bin/gpowers-config set cross_project_learnings false`
...
Check if the diff touches frontend files using `gpowers-diff-scope`:

```bash
source <($(gpowers-path home)/bin/gpowers-diff-scope <base> 2>/dev/null)
```
...
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <($(gpowers-path home)/bin/gpowers-diff-scope <base> 2>/dev/null) || true
...
```bash
$(gpowers-path home)/bin/gpowers-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `$(gpowers-path home)/review/specialists/testing.md`
2. **Maintainability** — read `$(gpowers-path home)/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `$(gpowers-path home)/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `$(gpowers-path home)/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `$(gpowers-path home)/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `$(gpowers-path home)/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `$(gpowers-path home)/review/design-checklist.md`

### Adaptive gating

After scope-based selection, apply adaptive gating based on specialist hit rates:

For each conditional specialist that passed scope gating, check the `gpowers-specialist-stats` output above:
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `$(gpowers-path home)/review/specialists/red-team.md`
...
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
OLD_CFG=$($(gpowers-path home)/bin/gpowers-config get codex_reviews 2>/dev/null || true)
...
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
$(gpowers-path home)/bin/gpowers-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
3. **Queue-aware version pick (workspace-aware ship, v1.6.4.0+).** Call `bin/gpowers-next-version` to see what's already claimed by open PRs + active sibling Conductor worktrees, then render the queue state to the user:

   ```bash
   QUEUE_JSON=$(bun run bin/gpowers-next-version \
     --base <base> \
     --bump "$BUMP_LEVEL" \
...
```bash
# Export [gpowers-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.gpowers"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(gpowers-path project)/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/$(gpowers-path home)/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**Always update the PR title to start with `v$NEW_VERSION`.** PR titles use the workspace-aware format `v<NEW_VERSION> <type>: <summary>` — version ALWAYS first, no exceptions, no "custom title kept intentionally" escape hatch. The shared helper `bin/gpowers-pr-title-rewrite.sh` is the single source of truth for the rule.

1. Read the current title: `CURRENT=$(gh pr view --json title -q .title)` (or `glab mr view -F json | jq -r .title`).
2. Compute the corrected title: `NEW_TITLE=$($(gpowers-path home)/bin/gpowers-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
# (See Step 19 idempotency block + bin/gpowers-pr-title-rewrite.sh for the rule.)
...
Append to `$(gpowers-path project)/$BRANCH-reviews.jsonl`:

```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> $(gpowers-path project)/$BRANCH-reviews.jsonl
```
