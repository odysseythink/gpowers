---
name: gpowers-health
description: "Code quality dashboard. Wraps existing project tools (type checker, linter, (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/health/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"health","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"health","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"health","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
queue_component:   10 if $(gpowers-path home)/.brain-queue.jsonl has <10 lines;
                    7 if 10-100; 0 if >=100 (suggests secret-scan rejections
                    piling up). N/A if artifacts_sync_mode == off.
push_component:    10 if (now - mtime of $(gpowers-path home)/.brain-last-push) < 24h;
...
Append one JSONL line to `$(gpowers-path project)/health-history.jsonl`:
...
Read the last 10 entries from `$(gpowers-path project)/health-history.jsonl` (if the
file exists and has prior entries).

```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)" && mkdir -p $(gpowers-path project)
tail -10 $(gpowers-path project)/health-history.jsonl 2>/dev/null || echo "NO_HISTORY"
```
