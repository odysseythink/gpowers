---
name: gpowers-autoplan
description: "Auto-review pipeline — reads the full CEO, design, eng, and DX review skills from disk (gpowers adapter for Kimi Code)"
gpowers-source: roles/skills/autoplan/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"autoplan","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"autoplan","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
Read the `/office-hours` skill file at `$(gpowers-path home)/office-hours/SKILL.md` using the Read tool.
...
SLUG=$($(gpowers-path home)/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t $(gpowers-path project)/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t $(gpowers-path project)/*-design-*.md 2>/dev/null | head -1)
...
echo "RESTORE_PATH=$(gpowers-path project)/${BRANCH}-autoplan-restore-${DATETIME}.md"
```
...
- Discover design docs: `ls -t $(gpowers-path project)/*-design-*.md 2>/dev/null | head -1`
...
- `$(gpowers-path home)/plan-ceo-review/SKILL.md`
- `$(gpowers-path home)/plan-design-review/SKILL.md` (only if UI scope detected)
- `$(gpowers-path home)/plan-eng-review/SKILL.md`
- `$(gpowers-path home)/plan-devex-review/SKILL.md` (only if DX scope detected)
...
```bash
_TEL=$($(gpowers-path home)/bin/gpowers-config get telemetry 2>/dev/null || echo off)
source $(gpowers-path home)/bin/gpowers-codex-probe
...
- Test plan: generate artifact at `$(gpowers-path project)/{user}-{branch}-test-plan-{datetime}.md`
...
- [ ] Test plan artifact written to disk at $(gpowers-path project)/
- [ ] "NOT in scope" section written
- [ ] "What already exists" section written
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
TASKS_DIR="${HOME}/$(gpowers-path project)/projects/${SLUG:-unknown}"
...
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope):
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"plan-devex-review","timestamp":"'"$TIMESTAMP"'","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"TYPE","tthw_current":"TTHW","tthw_target":"TARGET","unresolved":N,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Dual voice logs (one per phase that ran):
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

$(gpowers-path home)/bin/gpowers-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope), also log:
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```

If Phase 3.5 ran (DX scope), also log:
```bash
$(gpowers-path home)/bin/gpowers-review-log '{"skill":"autoplan-voices","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
