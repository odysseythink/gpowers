---
name: gpowers-land-and-deploy
description: "Land and deploy workflow. Merges the PR, waits for CI and deploy, (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/land-and-deploy/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$($(gpowers-path home)/bin/gpowers-slug 2>/dev/null)"
if [ ! -f $(gpowers-path project)/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat $(gpowers-path project)/land-deploy-confirmed 2>/dev/null)
...
```bash
$(gpowers-path home)/bin/gpowers-review-read 2>/dev/null
```
...
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > $(gpowers-path project)/land-deploy-confirmed
```
Continue to Step 2.
...
```bash
cat $(gpowers-path home)/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
...
ls -t ~/.gpowers-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.gpowers-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `gpowers-diff-scope` to classify the changes:

```bash
eval $($(gpowers-path home)/bin/gpowers-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o "$(gpowers-path project)/canary/post-deploy.png"
```
...
Save report to `$(gpowers-path project)/canary/{date}-pr{number}-deploy.md`.
