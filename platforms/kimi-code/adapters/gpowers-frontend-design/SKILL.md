---
name: gpowers-frontend-design
description: "Unified anti-slop frontend design methodology for premium interface generation. Brief inference, Dial system, multi-framework detection, design system mapping, AI-Tells ban, pre-flight checklist. (gpowers adapter for Kimi Code)"
gpowers-source: core/skills/frontend-design/SKILL.md
gpowers-module: core
---
...
<!-- SOURCE: $GPOWERS_HOME/core/skills/frontend-design/SKILL.md -->
...
* **Fonts:** Self-host with `@font-face` + `font-display: swap`. In Next.js, use `next/font`. Never link Google Fonts via `<link>` in production.
...
* **LCP** < 2.5s. Hero image must be preloaded (`next/image priority` in Next.js, or `<link rel="preload">` in Vite).
...
For the full audit protocol, see `redesign.md` appendix.
