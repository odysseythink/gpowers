---
name: gpowers-landing-report
description: "Read-only queue dashboard for workspace-aware ship. Shows which VERSION slots (gpowers adapter for Kimi Code)"
gpowers-source: tools/skills/landing-report/SKILL.md
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"landing-report","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> $(gpowers-path analytics)/skill-usage.jsonl 2>/dev/null || true
fi
...
$(gpowers-path home)/bin/gpowers-timeline-log '{"skill":"landing-report","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$(gpowers-path home)/bin/gpowers-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
`bin/gpowers-next-version` utility `/ship` uses, but with nothing mutating.
...
for LEVEL in micro patch minor major; do
  bun run bin/gpowers-next-version \
    --base "$BASE_BRANCH" \
    --bump "$LEVEL" \
