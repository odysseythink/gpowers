...
| 2 | **Discipline-agent personas** | `roles/skills/oracle` + `docs/methodology/executor-patterns.md` | Revised — see design doc | (Oracle persona partially defined by Ultrawork — pull it out into role skill in this pass) | Scope reduced after platform audit: only Oracle ships as a skill; Sisyphus/Hephaestus/Prometheus/Librarian/Explore degraded to methodology guide. Kimi gets optional `--agent-file` adapter. |
